...
Slide titles, body copy, and stat-callout context strings must be consistent with these positions. State them plainly. Do not dramatize them.
...
### Mission
We exist to correct the common failures of modern performance marketing by uniting paid media craft with measurement rigor. We tie spend to business outcomes, report what is working and what is not, and treat stewardship of client budgets and relationships as a professional duty.
...
**Do not add a seventh hex without a documented reason.**
...
9. **No new colors without a token.** If you reach for a seventh hex, use the closest existing token instead. Add a new color only with a documented reason.
10. **Chart series = Teal, Slate, Black.** More series = data density problem, not a color problem. Red only for outliers.
11. **Bar+dot motif: maximum once per slide.** The teal-bar-red-dot signature is a signature, not a repeating decoration. Two on one slide reads as a repeating decoration, and the second one weakens the first. Roughly 1 in 3-4 slides across the deck.
...
- **Multiple times on one slide. Maximum once per slide, no exceptions.** Two on one slide reads as a repeating decoration, and the second one weakens the first. If you need a second separator, use whitespace or the bottom accent bar.
...
- **Cover slide:** No logo. The project/client name identifies the deck.
...
- **No logo on cover.** The project name identifies the deck.

### Layout I: Quote / Interstitial

- **Real quotes only (v3.3.0).** Anything set in quotation marks must be a real statement from a named person or a cited source: a client, a named team member, or a published document. Attribution is mandatory: name, title, organization. Never invent a quote, never present an internal slogan as a quote, and never attribute a paraphrase as a quote. If no real quote exists, do not use this layout; use Layout O (dark emphasis) with a stated finding instead.
...
- Attribution line: Futura 11pt, white, letter-spaced. Name, title, organization.
...
The visual system is set by Section 1-8. The voice that fills it is plain written English. The reader should notice the content, not the writing. If a sentence would sound out of place in an analyst's report, it does not belong on a Blackbird slide.
...
2. **Precise.** Name specific methods (MMM, incrementality, holdout testing, marginal return analysis) rather than vague capabilities.
...
6. **State complications.** If the data complicates the story, the slide says so, in the same flat tone as everything else.
7. **No em dashes.** Not in titles, body copy, labels, footnotes, or chart annotations. Use a period, comma, colon, or parentheses. This also applies to double hyphens standing in for em dashes.
8. **Complete, ordinary sentences.** No sentence fragments for drama ("Nobody has it."). No three-beat slogans ("Test, measure, concentrate."). No X-not-Y constructions ("Earned, not guessed." "Punctuation, not pattern."). If the idea is real, it survives being written as a normal sentence.
9. **No aphorisms.** A rule of thumb stated as a proverb reads as ad copy. Write the operating fact instead: not "Measurement rigor is the product" but "Every engagement includes an incrementality measurement plan."
10. **No personification.** Methods, colors, and deliverables do not act. Not "CRO closes the loop" but "We run landing page tests on the capture channels." Say who does what.
11. **No second-person sales hooks.** No imperatives aimed at the reader ("Hire the agency that..."), no narrating the reader's world back to them ("Your buyers ask LLMs..."). Describe the work; let the reader draw the conclusion.
12. **No manufactured candor.** Statements about what Blackbird will not do or will not promise are allowed only when they carry specific technical content, stated flatly. "We do not report platform-attributed revenue as ground truth" is information. "What we will not promise: deterministic attribution. Nobody has it." is a sales device wearing honesty as a costume.
13. **No invented quotes.** See Layout I. Quotation marks are reserved for real, attributed statements.

### Voice: Do

- "Holdout testing showed 98% of attributed conversions were organic."
- "Marginal CPA above $50K monthly spend is 4x the blended average."
- "We report incrementality by channel each quarter and reallocate against marginal return."
- "LinkedIn CPL rose 22% in March. We moved 30% of the budget to Google Brand."
- "We do not report platform-attributed revenue as ground truth. We use holdout tests to estimate incremental effect."

### Voice: Don't

- "We're passionate about driving results." (hype)
- "Best-in-class solutions for modern marketers." (hype)
- "We help unlock the full potential of your marketing." (vague)
- "We tell the truth about what is working and show the math behind it." (aphorism; formerly a Do example in this spec, now banned)
- "Earned, not guessed." (X-not-Y construction)
- "No bait and switch." (sales-floor idiom)
- "Nobody has it." (fragment for drama)
- "Test, measure, concentrate." (three-beat slogan)
- "Measurement rigor is the product; media and creative are how we apply it." (aphorism)
- "Hire the agency that will tell you what did not work." (second-person pitch)
- "CRO closes the loop." (personification, jargon)

### Slide Titles Carry the Point

Titles state the slide's finding, not the topic. A reader who only scans titles should understand the deck's argument. Guardrails (v3.3.0): a title is a statement of fact with a subject and a verb. No imperatives, no second person, no questions as hooks, no inverted-promise framing, no title that could double as ad copy.

- Good: "Holdout testing showed 98% of attributed conversions were organic"
- Good: "Marginal CPA is 4x higher above $50K monthly spend"
- Bad: "Test Results" (topic label on a findings slide)
- Bad: "Hire the agency that will tell you what did not work" (pitch, imperative, second person)
- Bad: "What we will not promise" (manufactured candor)

Categorical slides (TOC, divider, roster) use topic-label titles. Two-line titles are fine if needed for clarity. If a slide has no finding, use a topic label rather than inventing a conclusion.
...
3. **Don't hardcode a seventh hex.** If you reach for a color outside the system, use the closest existing token instead. Add a new color only with a documented reason.
...
8. **No top-edge bars on cards, no exceptions.** Cards stand on their outline, not on a colored bar. The bottom-of-slide accent and the bar+dot signature are the only sanctioned horizontal bars in the system. A teal bar across the top of a card duplicates one of those signatures inside a container.
...
10. **Bar+dot motif: max once per slide.** Maximum one. Roughly 1 in 3-4 slides. Two on one slide read as a repeating decoration, and each weakens the other.
...
1. **Title with the conclusion**, not "Next Steps." A title like "Interviews conclude May 16; build starts May 19" tells the audience what they are seeing.
...
| **Voice** | No hype words ("revolutionary", "best-in-class", "passionate"). Declarative sentences. No exclamation marks. No em dashes anywhere in the deck. No slogans, aphorisms, three-beat or X-not-Y constructions, personification, or second-person hooks. Every quotation is real and attributed. Run Section 9 as a per-slide check. |
...
- **Section 9 rewritten.** New principles: complete ordinary sentences (no dramatic fragments, three-beat slogans, or X-not-Y constructions), no aphorisms, no personification, no second-person sales hooks, no manufactured candor, no invented quotes. The Voice Do examples are now plain reporting statements; the old aphorism examples ("We tell the truth about what is working and show the math behind it", "We do not hide behind dashboards") moved to the Don't list.
- **Layout I requires a real, attributed quote** (name, title, organization). No real quote, no quote slide; use Layout O with a stated finding instead. The old fixed "BLACKBIRD PPC" attribution invited unattributed slogans styled as quotes.
- **Title guardrails.** Conclusion titles stay, but a title is a statement of fact with a subject and a verb: no imperatives, no second person, no pitch framing, no inverted-promise headlines ("What we will not promise").
- **Mission wording**: "tell the truth about what is working" is now "report what is working and what is not."
- Pre-Ship Checklist Voice and Titles rows expanded; Common Mistakes 22-24 added (em dashes, invented quotes, slogan copy).
- Styleguide prose swept to match: hero and section titles rewritten to descriptive labels, personified color language removed, the gallery's unattributed quote example replaced with an attributed client-quote placeholder.
...
- **Conflicts resolved toward the deliberate decision.** Red is a co-accent at 12-18% with the sanctioned text role (supersedes the 8-10% framing the deck fork inherited from v3.0). Icons default to black with teal opt-in (formalizing what the deck fork's black filled circles implied). Content slides keep the red eyebrow, not the teal section number. The corner bird mark stays retired.
- Known gap: the styleguide's ten-frame slide gallery still renders v3.0 content-slide chrome (teal section number, corner mark). Refresh queued.

**v3.1, repo fork (June 24, 2026)**: red co-accent + text role, black icons, bigger photo library
...
**v3.1, deck fork (June 11, 2026)**: reporting-deck refinements from the Customer.io weekly agenda build
...
- **No top-edge bars on cards, no exceptions.** Removed from `addCard` helper, Layout B, Layout O stat cards, and all card layouts. Cards stand on their outline alone.
